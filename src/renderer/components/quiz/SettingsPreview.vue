  ...
  <v-card>
    ...
    <v-card-text>
      ...
      <v-list v-if="!isLoading" lines="two" density="compact">
        ...
        <v-list-item :title="$t('settingsPreview.questionAmount')" :subtitle="settings.questionAmount"></v-list-item>
        <v-list-item :title="$t('settingsPreview.testStyle')" :subtitle="settings.testStyle"></v-list-item>
        <v-list-item :title="$t('settingsPreview.questionStyle')">
          <template #subtitle>
            <v-chip v-for="style in settings.questionStyle" :key="style" size="small" class="mr-1 mt-1">
              {{ style }}
            </v-chip>
            <span v-if="!settings.questionStyle || settings.questionStyle.length === 0">{{ $t('settingsPreview.notSet') }}</span>
          </template>
        </v-list-item>

        <!-- AI Settings -->
        <v-list-subheader>{{ $t('settingsPreview.aiSettings') }}</v-list-subheader>
        <v-list-item :title="$t('settingsPreview.aiSource')" :subtitle="settings.llmSource"></v-list-item>
        
        <!-- OpenAI Specific -->
        <template v-if="settings.llmSource === 'OpenAI'">
          <v-list-item :title="$t('settingsPreview.model')" :subtitle="settings.openaiModel"></v-list-item>
          <v-list-item :title="$t('settingsPreview.reasoningEffort')" :subtitle="settings.reasoningEffort"></v-list-item>
        </template>
        
        <!-- LM Studio Specific -->
        <template v-if="settings.llmSource === 'LMStudio'">
          <v-list-item :title="$t('settingsPreview.model')" :subtitle="settings.lmStudioModel"></v-list-item>
        </template>
      </v-list>
